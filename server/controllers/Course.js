...
exports.editCourse=async(req,res)=>{
    try{
        const {courseTitle,courseDescription,category,price,whatYouWillLearn,instructions,tag,courseID,status}=req.body;
...
exports.createCourse=async(req,res)=>{
    try{
        //DATA FETCHING 
        const {courseTitle,courseDescription,whatYouWillLearn,price,category,tag,instructions}=req.body;
        ...
        if(!courseTitle || !courseDescription || !whatYouWillLearn || !price || !category || !tag){
            return res.status(500).json({
                success:false,
...
exports.getCoursePlusProgressDetails = async (req, res) => {
    try {
        ...
        const completedLectures = courseProgress.completesVideos?courseProgress.completesVideos:[];
        ...
        console.error("An error occurred while fetching the course info", error);
        return res.status(500).json({
            success: false,
            message: "Course info fetching failed",
        });
